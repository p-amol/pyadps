...
class EnsembleConfig(NamedTuple):
    ...
    num_datatypes: int = 8  # Updated from 7 to include Status (0x0500)
    beams: int = 4
    cells: int = 30
...
class VariableLeaderData(NamedTuple):
    ...
    temperature: int = 2000  # Temperature in 1/100ths Ã‚Â°C (bytes 20-21)
...
def build_ensemble(
    ...
    status_size = 2 + (1 * beams * cells)  # Status (NEW)
    ...
    # Add offset array
    offset_array = struct.pack("<HHHHHHHH", *offsets)  # Updated to 8 offsets
    ...
    data_sections += build_data_section(
        data_ids[6], status_size - 2, beams, cells
    )  # NEW Status
    data_sections += build_data_section(data_ids[7], bt_size - 2, beams, cells)
