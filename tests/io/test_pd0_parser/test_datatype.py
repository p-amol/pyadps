"""
Comprehensive pytest test suite for pd0parser.datatype function.
...
# ============================================================================
# TESTS: Cell and Beam Array Passed Directly (Lines 1015-1017)
# ============================================================================


class TestDatatypeCellBeamArrays:
    """Test datatype when cell and beam arrays are passed directly.

    These tests cover lines 1015-1017 in pd0_parser.py:
        else:
            cell_array = cell
            beam_array = beam

    When cell and beam are passed as arrays (not int), they should be used
    directly instead of fetching from fixedleader.
    """

    def test_cell_beam_arrays_passed_directly(self, valid_rdi_file):
        """Test that cell and beam arrays are used when passed as arrays.

        When cell and beam are numpy arrays, datatype should use them directly
        instead of calling fixedleader to fetch them.
        """
        # First get fileheader params
        dt, byte, byteskip, offset, idarray, ens_count, err = fileheader(
            str(valid_rdi_file)
        )
        assert err == 0

        # Create cell and beam arrays manually
        cell_array = np.array([30])  # 30 cells
        beam_array = np.array([4])  # 4 beams

        # Call datatype with arrays instead of ints
        result = datatype(
            str(valid_rdi_file),
            "velocity",
            cell=cell_array,
        ...
        data, ensemble, ret_cell, ret_beam, error_code = unpack_datatype_result(result)

        assert error_code == 0
        assert ensemble == ens_count
        # Returned arrays should be the ones we passed
        np.testing.assert_array_equal(ret_cell, cell_array)
        np.testing.assert_array_equal(ret_beam, beam_array)

    def test_cell_beam_arrays_multi_ensemble(self, multi_ensemble_rdi_file):
        """Test cell and beam arrays with multiple ensembles."""
        dt, byte, byteskip, offset, idarray, ens_count, err = fileheader(
            str(multi_ensemble_rdi_file)
        )
        assert err == 0
        assert ens_count == 3

        # Create arrays for 3 ensembles
        cell_array = np.array([30, 30, 30])
        beam_array = np.array([4, 4, 4])
...
# ============================================================================
# TESTS: Fixedleader Error Propagation (Lines 1013-1014)
# ============================================================================


class TestDatatypeFixedleaderErrorPropagation:
    """Test that errors from fixedleader are propagated.

    These tests cover lines 1013-1014 in pd0_parser.py:
        if fl_error_code != 0:
            error_code = fl_error_code
    """

    def test_fixedleader_error_propagated(self, tmp_path):
        """Test that non-zero error from fixedleader is propagated to datatype.

        Create a file where fixedleader will return an error but the file
        can still be partially processed.
        """
        # Create a file with corrupted Fixed Leader ID in second ensemble
        from .fixtures.ensemble_builder import build_ensemble
        import struct

        first_ensemble = build_ensemble()
        second_ensemble = bytearray(build_ensemble())

        # Corrupt FL ID in second ensemble
        fl_offset_in_header = 6
        fl_position = struct.unpack(
            "<H", second_ensemble[fl_offset_in_header : fl_offset_in_header + 2]
        )[0]
        second_ensemble[fl_position : fl_position + 2] = struct.pack("<H", 999)

        # Recalculate checksum
        byte_count = struct.unpack("<H", second_ensemble[2:4])[0]
        checksum = sum(second_ensemble[:byte_count]) & 0xFFFF
        second_ensemble[byte_count : byte_count + 2] = struct.pack("<H", checksum)

        rdi_file = tmp_path / "test_fl_error_propagation.000"
        rdi_file.write_bytes(first_ensemble + bytes(second_ensemble))

        # Get fileheader - should succeed for both ensembles since we fixed checksum
        dt, byte, byteskip, offset, idarray, ens_count, fh_err = fileheader(
            str(rdi_file)
        )

        # Modify idarray to claim FL exists (so fixedleader tries to read it)
        fake_idarray = idarray.copy()
        fake_idarray[1, 0] = 0  # Claim second ensemble has FL at first position

        # Call datatype - it will call fixedleader internally since we don't pass cell/beam
        # fixedleader will fail on second ensemble, returning ID_NOT_FOUND
        result = datatype(
            str(rdi_file),
            "velocity",
            byteskip=byteskip,
        ...
        # Error from fixedleader should be propagated
        assert error_code == ErrorCode.ID_NOT_FOUND.code


# ============================================================================
# TESTS: File Open Error After Geometry Setup (Lines 1038-1045)
# ============================================================================


class TestDatatypeFileOpenError:
    """Test datatype when file open fails after geometry is set up.

    These tests cover lines 1038-1045 in pd0_parser.py:
        if bfile is None:
            return (
    ...
    def test_file_open_fails_after_params_set(self, valid_rdi_file):
        """Test that file open error returns proper empty arrays.

        This tests the case where:
        1. fileheader succeeds
        2. fixedleader succeeds (or cell/beam passed directly)
        3. But then the third file open (in datatype's main loop) fails
        """
        # Get valid params first
        dt, byte, byteskip, offset, idarray, ens_count, err = fileheader(
            str(valid_rdi_file)
        )
        assert err == 0

        # Create cell/beam arrays so we skip fixedleader call
        cell_array = np.array([30])
        beam_array = np.array([4])

        # Mock safe_open to fail on the third call (datatype's main read)
        # Note: fileheader already ran, so we just need to fail the next open
        original_safe_open = None

        def mock_safe_open_fail(filename, mode="rb"):
            # Always fail - simulating file becoming inaccessible
            return (None, ErrorCode.IO_ERROR)

        with mock.patch(
            "pyadps.io.pd0_parser.safe_open", side_effect=mock_safe_open_fail
        ):
            result = datatype(
                str(valid_rdi_file),
                "velocity",
        ...
        # Should return error with empty arrays
        assert error_code == ErrorCode.IO_ERROR.code
        assert ensemble == 0
        assert len(data) == 0
        assert len(ret_cell) == 0
        assert len(ret_beam) == 0


# ============================================================================
# TESTS: Struct Parsing Errors in Data Loop (Lines 1113-1120)
# ============================================================================


class TestDatatypeStructParsingErrors:
    """Test datatype handling of struct parsing errors in the data loop.

    These tests cover lines 1113-1120 in pd0_parser.py:
        except (ValueError, StructError, OverflowError) as e:
            logger.error(f"Failed to parse {var_name} data...")
            error = ErrorCode.FILE_CORRUPTED
            ensemble = ensemble_idx
    """

    def test_value_error_during_data_parsing(self, valid_rdi_file):
        """Test that ValueError during data parsing returns FILE_CORRUPTED."""
        import struct
        ...
        # Mock np.frombuffer to raise ValueError
        original_frombuffer = np.frombuffer

        def mock_frombuffer(buffer, dtype):
            raise ValueError("Mocked ValueError during data parsing")

        with mock.patch("numpy.frombuffer", side_effect=mock_frombuffer):
            result = datatype(
                str(valid_rdi_file),
                "velocity",
                cell=cell_array,
    ...
    def test_overflow_error_during_data_parsing(self, valid_rdi_file):
        """Test that OverflowError during data parsing returns FILE_CORRUPTED."""
        dt, byte, byteskip, offset, idarray, ens_count, err = fileheader(
            str(valid_rdi_file)
        )
        assert err == 0
        ...
        def mock_frombuffer(buffer, dtype):
            raise OverflowError("Mocked OverflowError during data parsing")
    ...
    def test_struct_error_during_data_parsing(self, valid_rdi_file):
        """Test that struct.error during data parsing returns FILE_CORRUPTED.

        Note: struct.error (StructError) can be raised during binary data
        operations. We simulate this by raising it from frombuffer.
        """
        ...
        def mock_frombuffer(buffer, dtype):
            raise struct_module.error("Mocked struct.error during data parsing")
...
# ============================================================================
# TESTS: Error Code Final Assignment (Line 1126)
# ============================================================================


class TestDatatypeErrorCodeFinal:
    """Test that error.code is used when error occurred in loop.

    This tests line 1126:
        error_code_final = error.code

    This is hit when an error is detected during the parsing loop
    (not from fileheader/fixedleader).
    """

    def test_loop_error_sets_error_code_final(self, valid_rdi_file):
        """Test that error in parsing loop sets error_code_final correctly."""
        dt, byte, byteskip, offset, idarray, ens_count, err = fileheader(
            str(valid_rdi_file)
        )
        assert err == 0
        ...
        def mock_frombuffer(buffer, dtype):
            raise ValueError("Forced error to test error_code_final")
        ...
        # error_code_final should be FILE_CORRUPTED.code (8), not 0
        assert error_code == ErrorCode.FILE_CORRUPTED.code
        assert error_code != 0
