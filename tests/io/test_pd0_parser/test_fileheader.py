"""
Comprehensive pytest test suite for pd0parser.fileheader function.
...
@pytest.fixture
def file_with_invalid_second_ensemble_header_id(tmp_path, valid_rdi_ensemble):
    """Create file where second ensemble has invalid header ID (0x6F instead of 0x7F).

    First ensemble is valid and passes checksum verification.
    Second ensemble has corrupted header_id but valid structure otherwise.

    Tests line 456-459: ID_NOT_FOUND for non-first ensemble with bad header/source ID.
    """
    first_ensemble = valid_rdi_ensemble

    # Second ensemble with INVALID header_id (0x6F instead of 0x7F)
    second_ensemble_size = 100
    second_header = struct.pack(
        "<BBHBB",
        0x6F,  # INVALID header_id
        0x7F,  # Valid source_id
        second_ensemble_size,
        0,  # spare
        1,  # num_datatypes
    )
    second_header += struct.pack("<H", 6)  # One offset

    # Add padding to reach byte_count
    padding_needed = second_ensemble_size - len(second_header) + 6
    second_header += b"\x00" * padding_needed

    # Calculate and add checksum for second ensemble
    checksum = sum(second_header[:second_ensemble_size]) & 0xFFFF
    second_ensemble = second_header[:second_ensemble_size] + struct.pack("<H", checksum)

    rdi_file = tmp_path / "invalid_second_header_id.000"
    rdi_file.write_bytes(first_ensemble + second_ensemble)
    return rdi_file


@pytest.fixture
def file_with_invalid_second_ensemble_source_id(tmp_path, valid_rdi_ensemble):
    """Create file where second ensemble has invalid source ID (0x80 instead of 0x7F).

    First ensemble is valid and passes checksum verification.
    Second ensemble has valid header_id but corrupted source_id.

    Tests line 456-459: ID_NOT_FOUND for non-first ensemble with bad header/source ID.
    """
    first_ensemble = valid_rdi_ensemble

    # Second ensemble with valid header_id but INVALID source_id
    second_ensemble_size = 100
    second_header = struct.pack(
        "<BBHBB",
        0x7F,  # Valid header_id
        0x80,  # INVALID source_id (should be 0x7F)
        second_ensemble_size,
        0,
        1,
    )
    second_header += struct.pack("<H", 6)
    ...
    rdi_file = tmp_path / "invalid_second_source_id.000"
    rdi_file.write_bytes(first_ensemble + second_ensemble)
    return rdi_file
...
# ============================================================================
# TESTS: Second Ensemble Invalid Header/Source ID (Line 456-459 Coverage)
# ============================================================================


class TestFileheaderSecondEnsembleInvalidID:
    """Test fileheader handling when second ensemble has invalid header/source ID.

    These tests cover lines 456-459 in pd0_parser.py:
        else:  # i > 0 (non-first ensemble)
    ...
    def test_second_ensemble_invalid_header_id(
        self, file_with_invalid_second_ensemble_header_id
    ):
        """Test invalid header_id (0x6F) in second ensemble returns ID_NOT_FOUND.

        Scenario: First ensemble is valid, second has header_id=0x6F (not 0x7F).
        Expected: Truncate to 1 valid ensemble, return ID_NOT_FOUND error code.
        """
        datatype, byte, byteskip, address_offset, dataid, ensemble, error_code = (
            fileheader(file_with_invalid_second_ensemble_header_id)
        )

        # Should have parsed only the first valid ensemble
        assert ensemble == 1
        # Error code should be ID_NOT_FOUND (code 6)
        assert error_code == ErrorCode.ID_NOT_FOUND.code
        # Arrays should have data from first ensemble only
        assert len(datatype) == 1
        assert len(byte) == 1
        assert len(byteskip) == 1
        # First ensemble data should be valid
        assert datatype[0] == 7  # 7 datatypes from valid_rdi_ensemble

    def test_second_ensemble_invalid_source_id(
        self, file_with_invalid_second_ensemble_source_id
    ):
        """Test invalid source_id (0x80) in second ensemble returns ID_NOT_FOUND.

        Scenario: First ensemble valid, second has source_id=0x80 (not 0x7F).
        Expected: Truncate to 1 valid ensemble, return ID_NOT_FOUND error code.
        """
        datatype, byte, byteskip, address_offset, dataid, ensemble, error_code = (
            fileheader(file_with_invalid_second_ensemble_source_id)
        )

        # Should have parsed only the first valid ensemble
        assert ensemble == 1
        # Error code should be ID_NOT_FOUND
        assert error_code == ErrorCode.ID_NOT_FOUND.code
        # Arrays should have data from first ensemble only
        assert len(datatype) == 1
        assert len(byte) == 1
...
# ============================================================================
# TESTS: Struct Unpack Error (Line 473-485 Coverage)
# ============================================================================


class TestFileheaderStructUnpackError:
    """Test fileheader handling when struct.unpack fails on offset data.

    These tests cover lines 473-485 in pd0_parser.py:
        try:
            ...
            return (empty arrays..., 0, error_code)
    """

    def test_unpack_failure_returns_file_corrupted(self, tmp_path, valid_rdi_ensemble):
        """Test that struct.unpack failure returns FILE_CORRUPTED error.

        Uses mocking to force struct.unpack to raise an exception during
        offset data parsing. This covers the except block at line 473.
        """
        rdi_file = tmp_path / "test_unpack_mock.000"
        rdi_file.write_bytes(valid_rdi_ensemble)
        ...
        def mock_unpack(fmt, buffer):
            call_count[0] += 1
            # First call is for header (<BBHBB), let it pass
            if call_count[0] == 1:
                return original_unpack(fmt, buffer)
            # Second call is for offset array (H*n), make it fail
            if fmt.startswith("H"):
                raise struct.error("Mocked unpack failure for offset array")
            return original_unpack(fmt, buffer)
        ...
        assert error_code == ErrorCode.FILE_CORRUPTED.code
        # Should return empty arrays and ensemble count 0
        assert ensemble == 0
        assert len(datatype) == 0
        assert len(byte) == 0
        assert len(byteskip) == 0

    def test_unpack_value_error_returns_file_corrupted(
        self, tmp_path, valid_rdi_ensemble
    ):
        """Test that ValueError in unpack returns FILE_CORRUPTED.

        The except block catches Exception, so we test with ValueError too.
        """
        rdi_file = tmp_path / "test_unpack_value_error.000"
        rdi_file.write_bytes(valid_rdi_ensemble)
        ...
        def mock_unpack(fmt, buffer):
            ...
            if call_count[0] == 1:
                return original_unpack(fmt, buffer)
            if fmt.startswith("H"):
                raise ValueError("Mocked ValueError in unpack")
            return original_unpack(fmt, buffer)

        with mock.patch("pyadps.io.pd0_parser.unpack", side_effect=mock_unpack):
            result = fileheader(rdi_file)

        assert result[6] == ErrorCode.FILE_CORRUPTED.code
        assert result[5] == 0  # ensemble count
