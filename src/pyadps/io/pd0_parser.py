"""
pd0_parser.py - ADCP Binary File Reader

Module for reading and parsing Teledyne RDI Acoustic Doppler Current Profiler (ADCP)
...
class ErrorCode(Enum):
    ...
    Provides standardized error reporting throughout the pd0_parser module.
    Each member has a numeric code and descriptive message accessible via
    the `code` and `message` attributes.

    Attributes
    ----------
    code : int
        Numeric error code (0 = success, >0 = error/warning).
    message : str
    ...
    IO_ERROR : (3, "IO Error: Unable to open file.")
        General I/O failure during file operations.
    OUT_OF_MEMORY : (4, "Error: Out of memory.")
        Insufficient memory to complete operation.
    WRONG_ADCPFILE_TYPE : (5, "Error: Wrong ADCP File Type.")
        File header does not match Teledyne RDI PD0 format (header ID != 0x7F7F).
    ID_NOT_FOUND : (6, "Error: Data type ID not found.")
        Requested data type ID not present in file structure.
    DATATYPE_MISMATCH : (7, "Warning: Data type mismatch.")
        Data type count changed between ensembles (partial data returned).
    FILE_CORRUPTED : (8, "Warning: File Corrupted.")
        File structure invalid or truncated (partial data may be returned).
    ...
    CHECKSUM_ERROR : (10, "Error: Ensemble checksum verification failed.")
        Ensemble checksum does not match calculated value (per ADCP manual).
    UNKNOWN_ERROR : (99, "Unknown error.")
        Unspecified or unexpected error occurred.

    Examples
    --------
    Check error code after function call:

    >>> from pyadps.io.pd0_parser import fileheader, ErrorCode
    >>> _, _, _, _, _, ensemble, error_code = fileheader("test.000")
    >>> if error_code == ErrorCode.SUCCESS.code:
    ...     print(f"Successfully read {ensemble} ensembles")
    ... else:
    ...     print(f"Error: {ErrorCode.get_message(error_code)}")

    Use with safe_open for file operations:

    >>> from pyadps.io.pd0_parser import safe_open, ErrorCode
    >>> bfile, error = safe_open("data.pd0")
    >>> if error == ErrorCode.SUCCESS:
    ...     data = bfile.read()
    ...     bfile.close()
    ... elif error == ErrorCode.FILE_NOT_FOUND:
    ...     print("File does not exist")

    Retrieve message from error code:

    >>> ErrorCode.get_message(0)
    'Success'
    >>> ErrorCode.get_message(5)
    'Error: Wrong ADCP File Type.'

    Retrieve code from message:

    >>> ErrorCode.get_code("Success")
    0
    >>> ErrorCode.get_code("Error: File not found.")
    1
    """
    ...
    def get_code(cls, message: str) -> int:
        ...
        >>> code = pd0_parser.ErrorCode.get_code("Success")
        >>> print(code)
        0

        >>> code = pd0_parser.ErrorCode.get_code("Error: File not found.")
        >>> print(code)
        1

        >>> code = pd0_parser.ErrorCode.get_code("Unknown message")
        >>> print(code)
        99
        """
...
# ============================================================================
# ERROR RETURN HELPERS (Private - Internal Use Only)
# ============================================================================


def _empty_fileheader_result(error_code: int) -> FileHeaderReturn:
    """
    Create an empty FileHeaderReturn tuple for fatal error conditions.

    Used when fileheader() encounters unrecoverable errors (file not found,
    wrong file type, etc.) and cannot return any valid data.

    Args
    ----
    error_code : int
        Error code to include in the return tuple.

    Returns
    -------
    FileHeaderReturn
        7-tuple with empty arrays, zero ensemble count, and error code:
...
def _empty_datatype_result(error_code: int) -> DataTypeReturn:
    """
    Create an empty DataTypeReturn tuple for fatal error conditions.

    Used when datatype() encounters unrecoverable errors and cannot
    ...
    -------
    DataTypeReturn
        5-tuple with empty arrays, zero ensemble count, and error code:
        (data, ensemble, cell_array, beam_array, error_code)
    """
    return (
        np.array([], dtype=np.int16),  # data
        0,  # ensemble
        np.array([], dtype=np.int32),  # cell_array
        np.array([], dtype=np.int32),  # beam_array
        error_code,
    )
...
def _calculate_checksum(data: bytes) -> int:
    """
    Calculate ensemble checksum per ADCP manual.
...
def fileheader(rdi_file: FilePathType) -> FileHeaderReturn:
    """
    Parse ADCP file header to extract ensemble metadata.
    ...
    - Checksum verification is performed per ADCP manual.
    ...
    if bfile is None:
        return _empty_fileheader_result(error.code)
        ...
        while byt := bfile.read(6):
            ...
            if i == 0:
                if headerid[0] != 127 or sourceid[0] != 127:
                    error = ErrorCode.WRONG_ADCPFILE_TYPE
            ...
            # CHECKSUM VERIFICATION (Last step - per ADCP manual)
...
def fixedleader(
    ...
    """
    Extract Fixed Leader data from ADCP binary file.
...
def variableleader(
    ...
    """
    Extract Variable Leader data from ADCP binary file.
...
def datatype(
    ...
    """
    Extract 3D data arrays (velocity, correlation, echo, etc.) from ADCP binary file.
    ...
    if (
        ...
        if error_code > 0 and error_code < 6:
            return _empty_datatype_result(error_code)
    ...
    bfile, error = safe_open(filename, "rb")
    if bfile is None:
        return _empty_datatype_result(error.code)
    ...
    if not vid_tuple:
        ...
        return _empty_datatype_result(ErrorCode.VALUE_ERROR.code)
    ...
    if fbyteskip is None:
        ...
        return _empty_datatype_result(ErrorCode.ID_NOT_FOUND.code)
