"""
Comprehensive pytest test suite for pd0parser.variableleader function.
...
class TestVariableleaderEnvironmentalFields:
    ...
    def test_salinity_field_extraction(self, valid_rdi_file_with_vl):
        ...
        assert error_code == 0
        # Salinity packed as signed int16 in pd0parser format
...
# ============================================================================
# TESTS: VL ID After Read (Lines 829-833)
# ============================================================================


class TestVariableleaderIDAfterRead:
    """Test variableleader handling when VL ID is invalid after reading bdata.

    These tests cover lines 829-833 in pd0_parser.py:
    ...
    This is different from the idarray lookup failure (line 818-822) because
    here the VL section is found in idarray but the actual VL ID bytes
    are corrupted when read from the file.
    """

    @pytest.fixture
    def rdi_file_with_invalid_vl_id_after_read(self, tmp_path):
        """Create file where Variable Leader ID is invalid AFTER bdata is read.

        The trick: We corrupt the VL ID bytes in the file, but we'll pass a
        manually constructed idarray to variableleader() that claims the VL section
        exists (contains 128). When variableleader reads the actual bytes, it finds 999.
        """
        ensemble_bytes = build_ensemble()
        data = bytearray(ensemble_bytes)

        # Find Variable Leader section position from the offset array
        # Header: BBHBB (6 bytes), then offset array (2 bytes per datatype)
        # VL is the second datatype (index 1)
        vl_offset_in_header = 6 + (2 * 1)  # Second offset points to Variable Leader
        vl_position = struct.unpack(
            "<H", data[vl_offset_in_header : vl_offset_in_header + 2]
        )[0]

        # Corrupt the VL ID to 999 (not 128 or 129)
        data[vl_position : vl_position + 2] = struct.pack("<H", 999)
        ...
        rdi_file = tmp_path / "test_vl_invalid_id_after_read.000"
        rdi_file.write_bytes(bytes(data))
        return rdi_file

    def test_invalid_vl_id_after_read_returns_id_not_found(
        self, rdi_file_with_invalid_vl_id_after_read
    ):
        """Test that invalid VL ID (999) after reading bdata returns ID_NOT_FOUND.

        Scenario: We pass a fake idarray that claims VL section exists (has 128),
        but the actual VL ID bytes in the file are corrupted to 999.
        ...
        dt, byte, byteskip, offset, idarray, ens_count, err = fileheader(
            str(rdi_file_with_invalid_vl_id_after_read)
        )

        # The fileheader will have read 999 as the dataid, not 128.
        # We need to manually create an idarray that has 128 so variableleader
        # will try to read the VL section.
        fake_idarray = idarray.copy()
        fake_idarray[0, 1] = 128  # Claim second datatype is Variable Leader (ID 128)

        # Call variableleader with the fake idarray
        data, ensemble_count, error_code = variableleader(
            str(rdi_file_with_invalid_vl_id_after_read),
            byteskip=byteskip,
            offset=offset,
        ...
        # Should detect invalid ID (999 != 128 and != 129) and return error
...
# ============================================================================
# TESTS: Struct Parsing Errors (Lines 894-901)
# ============================================================================


class TestVariableleaderStructParsingErrors:
    """Test variableleader handling of struct parsing errors.

    These tests cover lines 894-901 in pd0_parser.py:
        except (ValueError, StructError, OverflowError) as e:
            logger.error(f"Failed to parse Variable Leader at ensemble {i + 1}...")
            error = ErrorCode.FILE_CORRUPTED
            ensemble = i
    """

    def test_struct_error_during_vl_parsing(self, tmp_path):
        """Test that struct.error during VL parsing returns FILE_CORRUPTED.

        Uses mocking to force struct.unpack to raise struct.error on a specific
        format string used inside the variableleader try block.
        """
        ensemble_bytes = build_ensemble()
        rdi_file = tmp_path / "test_vl_struct_error.000"
        rdi_file.write_bytes(ensemble_bytes)

        # Get fileheader params first
        dt, byte, byteskip, offset, idarray, ens_count, err = fileheader(str(rdi_file))
        assert err == 0

        original_unpack = struct.unpack

        def mock_unpack(fmt, buffer):
            # Fail on the ADC channels unpack format (8 bytes)
            # This is deep enough in the try block to be caught by the except
            if fmt == "<BBBBBBBB" and len(buffer) == 8:
                raise struct.error("Mocked struct.error during VL parsing")
    ...
    def test_value_error_during_vl_parsing(self, tmp_path):
        """Test that ValueError during VL parsing returns FILE_CORRUPTED."""
        ensemble_bytes = build_ensemble()
        rdi_file = tmp_path / "test_vl_value_error.000"
        rdi_file.write_bytes(ensemble_bytes)
        ...
        def mock_unpack(fmt, buffer):
            # Fail on the pressure fields unpack format
            if fmt == "<HiiB":
                raise ValueError("Mocked ValueError during VL parsing")
            return original_unpack(fmt, buffer)
    ...
    def test_overflow_error_during_vl_parsing(self, tmp_path):
        """Test that OverflowError during VL parsing returns FILE_CORRUPTED."""
        ensemble_bytes = build_ensemble()
        rdi_file = tmp_path / "test_vl_overflow_error.000"
        rdi_file.write_bytes(ensemble_bytes)
        ...
        def mock_unpack(fmt, buffer):
            # Fail on the sensor data unpack format
            if fmt == "<HHHhhHh":
                raise OverflowError("Mocked OverflowError during VL parsing")
            return original_unpack(fmt, buffer)
...
# ============================================================================
# TESTS: File Seeking Errors (Lines 903-909)
# ============================================================================


class TestVariableleaderFileSeekingErrors:
    """Test variableleader handling of file seeking errors.

    These tests cover lines 903-909 in pd0_parser.py:
    ...
    def test_oserror_during_seek_returns_file_corrupted(self, tmp_path):
        """Test that OSError during bfile.seek returns FILE_CORRUPTED."""
        import io

        ensemble_bytes = build_ensemble()
        rdi_file = tmp_path / "test_vl_oserror_seek.000"
        rdi_file.write_bytes(ensemble_bytes)
        ...
        def mock_open_with_failing_seek(*args, **kwargs):
            ...
            def failing_seek(offset_val, whence=0):
                ...
                # Fail on subsequent seeks in the VL parsing loop
                if seek_call_count[0] > 1:
                    raise OSError("Mocked OSError during seek in VL loop")
    ...
    def test_unsupported_operation_during_seek_returns_file_corrupted(self, tmp_path):
        """Test that io.UnsupportedOperation during seek returns FILE_CORRUPTED."""
        import io

        ensemble_bytes = build_ensemble()
        rdi_file = tmp_path / "test_vl_unsupported_seek.000"
        rdi_file.write_bytes(ensemble_bytes)
        ...
        def mock_open_with_unsupported_seek(*args, **kwargs):
            f = original_open(*args, **kwargs)
            original_seek = f.seek
            seek_call_count = [0]
            ...
            def failing_seek(offset_val, whence=0):
                ...
                if seek_call_count[0] > 1:
                    raise io.UnsupportedOperation(
                        "Mocked UnsupportedOperation during seek"
                    )
                return original_seek(offset_val, whence)
        ...
        with mock.patch("builtins.open", side_effect=mock_open_with_unsupported_seek):
            data, ensemble_count, error_code = variableleader(
                str(rdi_file),
                byteskip=byteskip,
                offset=offset,
    ...
    def test_oserror_on_second_ensemble_truncates_correctly(self, tmp_path):
        """Test OSError on second ensemble seek truncates to first valid ensemble."""
        import io

        ensemble_bytes = build_ensemble()
        rdi_file = tmp_path / "test_vl_oserror_second_ens.000"
        ...
        def mock_open_with_failing_seek(*args, **kwargs):
            ...
            def failing_seek(offset_val, whence=0):
                seek_call_count[0] += 1
                # Let first ensemble complete (several seeks)
                # Fail on 4th seek (during second ensemble)
                if seek_call_count[0] >= 4:
                    raise OSError("Mocked OSError on second ensemble seek")
                return original_seek(offset_val, whence)
        ...
        assert error_code == ErrorCode.FILE_CORRUPTED.code
        # First ensemble should have been parsed
        assert ensemble_count == 1
